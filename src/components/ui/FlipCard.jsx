...
export default function FlipCard({
  ...
  const onPointerMove = e => {
    ...
    if (g && g.id === e.pointerId) {
      ...
      const fallback = axis === 'x' ? height : width;
      const measured = rootRef.current
        ...
        : 0;
      const span =
        dragDistance > 0 ? dragDistance : measured || (typeof fallback === 'number' ? fallback : 300);
  ...
  const release = (e, cancelled) => {
    ...
    if (!g.moved) {
      if (cancelled) settle(target.current, 0, false);
      else if (flipOnClick) flip(false);
      else onActivate?.(e);
      return;
    }
    const here = turn.get();
